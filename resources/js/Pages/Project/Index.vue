            ...
            <tr v-for="(project, index) in projects?.data" :key="project.id" class="mb-4 cursor-pointer hover:bg-[#acababa8]" @click="$inertia.get(route('projects.show', project.id))">
                ...
                <td class="text-left py-2 flex space-x-1 items-center px-2">
                  ...
                  <div class="relative bg-[#D9D9D9] rounded-full h-5 w-24">
                    <div class="absolute top-0 left-0 bg-secondary h-5 rounded-full" :style="{ width: ( project.tasks.filter(task => task.status === 'Terminada').length / project.tasks.length) * 100 + '%' }"></div>

  ...
  <AppLayoutNoHeader title="Calidad">
    <div class="flex flex-col md:mx-9 md:my-7 space-y-3 m-1">
      ...
      <div class="flex justify-between">
        ...
        <div class="flex items-center space-x-2">
          <Link
            v-if="$page.props.auth.user.permissions.includes('Crear registro de calidad')"
    ...
    <div class="w-11/12 mx-8 my-16">
      <table v-if="filteredTableData.length" class="w-full mx-auto text-sm">
        ...
        <tbody>
          <tr
            v-for="quality in filteredTableData"
            :key="quality.id"
          ...
          >
            ...
            <td class="py-2">
              <span class="py-1 rounded-full">{{ quality.created_at }}</span>
            </td>
            <td class="py-2">
              <span class="py-1 rounded-full">{{ quality.folio }}</span>
            </td>
            <td class="py-2">
              {{ quality.status }}
            </td>
            <td
              v-if="
                $page.props.auth.user.permissions.includes('Eliminar registro de calidad')
              "

<template>
    <AppLayout title="Administrador de cajas">
        <template #header>
            <div class="flex justify-between">
                <div class="flex items-center space-x-2">
                    <h2 class="font-semibold text-xl leading-tight">Administrador de cajas</h2>
                </div>
...
methods: {
    ...
    async deleteSelections() {
        try {
            const response = await axios.post(route('boxes.massive-delete', {
                boxes: this.$refs.multipleTableRef.value
            }));
    ...
    handleRowClick(row) {
        this.$inertia.get(route('boxes.edit', row));
    },
    ...
    handleCommand(command) {
        ...
        if (commandName == 'clone') {
            ...
            this.$inertia.get(route('boxes.' + commandName, rowId));
        }
    },
},
computed: {
    filteredTableData() {
        if (!this.search) {
            return this.boxes.filter((item, index) => index >= this.start && index < this.end);
        } else {
            return this.boxes.filter(
                (box) =>
                    box.name.toLowerCase().includes(this.search.toLowerCase()) ||
                    box.serial_number.toLowerCase().includes(this.search.toLowerCase()) ||
                    box.supplier.toLowerCase().includes(this.search.toLowerCase())
            )
        }
    }
},

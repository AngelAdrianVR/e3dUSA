        ...
        <main class="flex flex-col md:mx-9 md:my-7 space-y-3 m-1 text-sm">
            ...
            <section class="flex justify-between">
                <div class="w-1/2 md:w-1/3 mr-2">
                    <el-select @change="$inertia.get(route('shippings.show', shippingRateSelected))" v-model="shippingRateSelected"
                        filterable placeholder="Buscar registro de logística" no-data-text="No hay opciones registradas"
                        no-match-text="No se encontraron coincidencias">
                        <el-option v-for="item in shipping_rates" :key="item" :label="item.id.toString().padStart(3, '0')" :value="item.id" />
                    </el-select>
                </div>
            ...
            <section class="pt-7 md:grid grid-cols-3 gap-8">
                <!-- lado izquierdo del grid -->
                <article class="col-span-1">
                    <h1 class="text-lg font-bold text-center">{{ shipping_rate.catalog_product?.name }}</h1>

                    <figure class="flex justify-center items-center mt-5 h-48 bg-gray-200 rounded-lg">
                    ...
                    <div v-for="(item, index) in shipping_rate.catalog_product?.shipping_rates" :key="index" class="border border-[#CCCCCC] py-4 rounded-lg mt-5">
                        <div class="flex justify-between px-7">
                            ...
                            <div class="flex items-center space-x-3">
                                <button class="size-7 flex items-center justify-center rounded-full text-primary bg-[#cecccc]">
...
data(){
    return {
        shippingRateSelected: this.shipping_rate.id
    }
},
components:{
    AppLayoutNoHeader,

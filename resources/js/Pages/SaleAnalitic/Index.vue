<template>
  <AppLayoutNoHeader title="Inicio CRM">
    <div class="md:mx-9 md:my-1 m-1">
      ...
      <!-- Estadistics -->
      <h2 class="text-primary lg:text-lg text-sm lg:mt-8 mt-6 font-bold">Estadísticas</h2>
      <div class="lg:grid grid-cols-1 gap-10 mt-4 space-y-4 lg:space-y-0">
        <PieChart v-for="(item, index) in pieChartOptions" :key="index" :options="item.data" :title="item.title" :icon="item.icon" />
      </div>
      <!-- analisis de ventas -->
      <h2 class="text-primary lg:text-lg text-sm lg:mt-20 mt-6 font-bold">Análisis de ventas</h2>

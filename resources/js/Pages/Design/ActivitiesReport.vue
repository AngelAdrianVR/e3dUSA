<template>
    ...
    <Head title="Reporte de actividades de diseño" />
    <!-- logo -->
    ...
    <div class="text-xs px-6 mt-8 relative">
        ...
        <h1 class="font-bold text-xl mx-5 flex items-center justify-between">
            <span>Actividades de diseñadores</span>
        ...
        <table class="w-full table-auto mt-4" v-if="Object.keys(data).length > 0">
            <thead>
                <tr class="text-left *:px-2 *:py-1">
                    ...
                    <th>Nombre</th>
                    <th>Ordenes</th>
                    <th>Tiempo total invertido</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(item, index) in Object.values(data)" :key="index"
                    class="*:px-2 *:py-1 *:align-top odd:bg-secondarylight">
                    <td>{{ item.designer_id }}</td>
                    <td>{{ item.designer_name }}</td>
                    <td>
                        <div v-for="(order, index2) in item.orders" :key="order.id" class="flex space-x-3">
                            <p class="w-[20%]">{{ (index2 + 1) }}. {{ order.name }}.</p>
                            <p class="w-[35%]">Inicio: {{ order.started_at }} Término: {{ order.finished_at }}</p>
                            <p class="w-[25%]">Tiempo invertido: {{ order.time_spent }}</p>
                        </div>
                    </td>
        ...
        <p v-else class="text-center text-lg mt-10">
            No hay información de órdenes de diseño para los diseñadores
            seleccionados
        </p>
    </div>
</template>

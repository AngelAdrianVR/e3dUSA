    ...
    <div class="flex flex-col md:mx-9 md:my-7 space-y-3 m-1">
      ...
      <div class="flex justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <el-tooltip content="Registrar Interaccion por llamada" placement="top">
    ...
    <div v-if="tabs == 1" class="md:grid grid-cols-2 gap-1 border-b-2 border-[#cccccc] text-sm">
      <div class="grid grid-cols-2 gap-1 text-left p-4 md:ml-10 border-r-2 border-gray-[#cccccc] items-center self-start">
        ...
        <a class="text-secondary hover:underline" :href="route('oportunities.show', call_monitor.data.oportunity?.id)">
          <span>{{ call_monitor.data.oportunity?.folio ?? '--' }} - {{ call_monitor.data.oportunity?.name ?? '--'
            }}</span>
        </a>
      ...
      <div class="grid grid-cols-2 gap-1 text-left p-4 md:ml-10 self-start">
        ...
        <p style="white-space: pre-line;">{{ call_monitor.data.notes ?? 'Sin descripción' }}</p>
      </div>
    </div>

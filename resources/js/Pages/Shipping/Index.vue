            ...
            <div class="w-11/12 lg:w-5/6 mx-auto mt-6">
                ...
                <el-table :data="filteredShippings" @row-click="handleRowClick" max-height="670" style="width: 100%"
                    ...
                    <el-table-column label="Productos" width="150">
                        <template #default="scope">
                            <p>
                                {{ scope.row.partialities ? [...new Set(scope.row.partialities.flatMap(p => p.productsSelected?.map(ps => ps.name) || []))].join(', ') : '- Sin productos -' }}
                            </p>
                        </template>
                    </el-table-column>
                    <el-table-column label="Tipo de entrega" width="130">
                        <template #default="scope">
                            <div class="flex items-center space-x-1">
                                <span v-html="getStatusIcon(scope.row.partialities)"></span>
                                <p>{{ scope.row.partialities?.length > 1 ? scope.row.partialities?.length + ' Parcialidades' : 'Única' }}</p>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="Guía" width="150">
                        <template #default="scope">
                            <p v-for="(p, index) in scope.row.partialities" :key="p.tracking_guide">
                                {{ p.tracking_guide ? 'Guía ' + (index + 1) + ': ' + p.tracking_guide 
                                    : 'Guía ' + (index + 1) + ': -'
                                }}
                            </p>
                        </template>
                    </el-table-column>
                    <el-table-column label="Fecha promesa de envío" width="115">
                        <template #default="scope">
                            <p v-for="p in scope.row.partialities" :key="p.promise_date">
                                {{ p.promise_date ? formatDate(p.promise_date) : '-'}}
                            </p>
                        </template>
                    </el-table-column>
                    <el-table-column label="Enviado el" width="120">
                        <template #default="scope">
                            <p v-for="p in scope.row.partialities" :key="p.sent_at">
                            ...
                            <el-dropdown trigger="click" @command="handleCommand">
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        ...
                                        <el-dropdown-item v-if="!scope.row.partialities?.every(item => item.sent_at)"
                                        ...
                                        <!-- <el-dropdown-item v-if="scope.row.partialities?.length > 1 && !scope.row.partialities?.every(item => item.sent_at)"
...
methods:{
    ...
    getStatusIcon(partialities) {
        //enviadas todas las parcialidades
        if (partialities?.every(item => item.sent_at)) {
        ...
        } else if ( partialities?.length > 1 && partialities?.some(item => item.sent_at) ) {
            //si tiene alguna parcialidad enviada

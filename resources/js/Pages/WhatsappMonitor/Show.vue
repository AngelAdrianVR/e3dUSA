<template>
  <AppLayoutNoHeader title="Interacciones WhastApp">
      ...
      <div class="flex justify-between">
        ...
        <div class="flex items-center space-x-2">
          <el-tooltip content="Editar Interaccion whatsapp" placement="top">
            ...
            <button class="size-9 rounded-lg bg-[#D9D9D9]">
              <i class="fa-solid fa-pen text-sm"></i>
            </button>
            </Link>
          </el-tooltip>
          <el-tooltip content="Registrar Interaccion whatsapp" placement="top">
    ...
    <div v-if="tabs == 1" class="md:grid grid-cols-2 gap-1 border-b-2 border-[#cccccc] text-sm">
      <div class="grid grid-cols-2 gap-1 text-left p-4 md:ml-10 border-r-2 border-gray-[#cccccc] items-center self-start *:self-start">
        <p class="text-secondary col-span-2 mb-2">Información de la oportunidad</p>
        <span class="text-gray-500">Folio de oportunidad</span>
        <a class="text-secondary hover:underline"
          :href="route('oportunities.show', whatsapp_monitor.data.oportunity?.id)">
          <span>{{ whatsapp_monitor.data.oportunity?.folio ?? '--' }} - {{ whatsapp_monitor.data.oportunity?.name ??
            '--'
            }}</span>
        </a>
      ...
      <div class="grid grid-cols-2 gap-1 text-left p-4 md:ml-10 self-start">
        ...
        <p style="white-space: pre-line;">{{ whatsapp_monitor.data.notes ?? 'Sin descripción' }}</p>
        <p class="text-secondary col-span-2 mb-2 mt-5">Archivos adjuntos</p>
        <div v-if="whatsapp_monitor.data.media?.length" class="col-span-full">
          <li v-for="file in whatsapp_monitor.data.media" :key="file"
        ...
        <p class="text-sm text-gray-400" v-else><i class="fa-regular fa-file-excel mr-3"></i>No hay archivos adjuntos
        </p>
      </div>
    </div>

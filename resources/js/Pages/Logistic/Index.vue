    ...
    <AppLayout title="Logística">
        <template #header>
            <div class="flex justify-between mt-4">
                ...
                <div class="flex space-x-2">
                    <button class="rounded-full bg-[#D9D9D9] text-[#4B5563] px-3 py-1 text-sm">Enviado</button>
        ...
        <div v-else class="relative overflow-hidden min-h-[60vh]">
            ...
            <div class="w-11/12 lg:w-5/6 mx-auto mt-6">
                ...
                <el-table :data="filteredLogistics" @row-click="handleRowClick" max-height="670" style="width: 100%"
                    @selection-change="handleSelectionChange" ref="multipleTableRef"
                    :row-class-name="tableRowClassName">
                    <el-table-column type="selection" width="30" />
                    <el-table-column label="Folio" width="100">
                        <template #default="scope">
                            <p>EV-{{ scope.row.id.toString().padStart(4, '0') }}</p>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="sale.user.name" label="Vendedor" />
                    <el-table-column label="Creado el">
                        <template #default="scope">
                            <p>{{ formatDate(scope.row.sale.created_at) }}</p>
                        </template>
                    </el-table-column>
                    <el-table-column prop="sale.company_branch.name" label="Cliente" />
                    <el-table-column label="Productos" width="150">
                        <template #default="scope">
                            <p>{{ scope.row.sale.productions?.length ? scope.row.sale.productions.map(p => p.catalog_product_company_sale.catalog_product_company.catalog_product.name).join(', ') : '- Sin producción -' }}</p>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="Guía" width="100">
                        <template #default="scope">
                            <p>{{ scope.row.sale.tracking_guide ?? '-' }}</p>
                        </template>
                    </el-table-column>
                    <el-table-column label="Fecha de entrega" width="115">
                        <template #default="scope">
                            <p>{{ scope.row.sale.promise_date ?? '-' }}</p>
                        </template>
                    </el-table-column>
...
data() {
    return {
        ...
        filteredLogistics: this.logistics.data,
        search: '',
        loading: false,
        // pagination: this.logistics,
    }
},
components:{
...
methods:{
    ...
    applyStatusFilter() {
        ...
        this.filteredLogistics = this.logistics.data.filter(sale =>
    ...
    handleCommand(command) {
        ...
        if (commandName == 'clone') {
            ...
            this.$inertia.get(route('logistics.' + commandName, rowId));
        }
    },
    fetchItemsFiltered() {
        if ( this.filter === 'Todas las órdenes' ) {
            this.$inertia.get(route('logistics.index-all'));
        }
    },
    ...
    handleRowClick(row) {
        this.$inertia.get(route('logistics.show', row));
    },
    ...
    async deleteSelections() {
        try {
            const response = await axios.post(route('logistics.massive-delete', {
                logistics: this.$refs.multipleTableRef.value
            }));
            ...
            if (response.status == 200) {
                ...
                // update list of logistics
                let deletedIndexes = [];
                this.logistics.data.forEach((sale, index) => {
                ...
                for (const index of deletedIndexes) {
                    this.logistics.data.splice(index, 1);
                }
    ...
    async handleSearch(search) {
        ...
        try {
            if (!this.search) {
                this.filteredLogistics = this.logistics.data;
            } else {
                const response = await axios.post(route('logistics.get-matches'), { query: this.search });
 
                if (response.status === 200) {
                    this.filteredLogistics = response.data.items.data;
                }
            }
        } catch (error) {
            console.log(error);

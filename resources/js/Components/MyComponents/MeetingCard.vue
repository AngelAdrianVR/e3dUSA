    ...
    <div class="h-auto py-3 px-2 border bg-[#d9d9d9] rounded-[20px] lg:rounded-lg">
        <template v-if="meetings.data.length">
            <div v-for="(meeting, index) in meetings.data" :key="meeting.id"
                ...
                <div>
                    <Link v-if="meeting.user.id == $page.props.auth.user.id" class="hover:underline"
                    ...
                    <p class="text-sm"><i class="fa-regular fa-clock mr-1"></i> {{ meeting.start.format }} - {{
                        meeting.end.format }} </p>
                ...
                <div
                    ...
                    <p v-if="meeting.user.id !== $page.props.auth.user.id">Asistenca:</p> <span
                        v-if="meeting.user.id !== $page.props.auth.user.id" class="lg:col-span-4 flex space-x-2">
                        <div
                            v-if="meeting.participants.find(item => item.id == $page.props.auth.user.id)?.pivot?.attendance_confirmation == 'Revisando'">
                            <el-popconfirm confirm-button-text="Si" cancel-button-text="No" icon-color="#0355B5"
                                title="¿Continuar?" @confirm="setAttendanceConfirmation('Confirmado', index)">
                                <template #reference>
                                    <button
                                        class="text-sm text-white bg-green-500 rounded-[10px] py-px px-2 mr-3 disabled:opacity-25 disabled:cursor-not-allowed"
                            ...
                            <el-popconfirm v-if="$page.props.auth.user.permissions.includes('Eliminar ordenes de venta')"
                                ...
                                <template #reference>
                                    <button
                                        class="text-sm text-white bg-red-500 rounded-[10px] py-px px-2 mr-3 disabled:opacity-25 disabled:cursor-not-allowed"

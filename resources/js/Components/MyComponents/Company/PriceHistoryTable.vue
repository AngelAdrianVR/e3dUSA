<template>
    <div v-for="catalogProductCompany in catalogProductsCompany" :key="catalogProductCompany"
        class="w-[700px] mx-auto relative mb-16">
        ...
        <div class="">
            <div class="mb-4 grid grid-cols-3">
                <p class="absolute -top-7 left-2 font-bold text-sm">Producto: <span
                        @click="$inertia.get(route('catalog-products.show', catalogProductCompany.id))"
                        class="underline text-secondary cursor-pointer">{{ catalogProductCompany.name }}</span></p>
                <PrimaryButton
                    @click="showUpdatePriceModal = true; form.product_company_id = catalogProductCompany.pivot.id"
                    class="absolute -top-7 right-2 font-bold text-sm !py-1">Actualizar precio</PrimaryButton>
    ...
    <!-- ----------------- modal ----------- -->
    <Modal :show="showUpdatePriceModal" @close="showUpdatePriceModal = false">

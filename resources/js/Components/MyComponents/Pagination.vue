<template>
  <div v-if="pagination.meta.links.length > 3" class="overflow-x-auto flex items-center justify-between my-1">
        ...
        <nav class="isolate inline-flex -space-x-px rounded-lg" aria-label="Pagination">
          ...
          <template v-for="(link, key) in pagination.meta.links">
            <div :key="key" v-if="link.url == null"
              class="rounded-sm relative inline-flex items-center border bg-gray-100 cursor-pointer hover:bg-gray-200 hover:border-secondary px-2 py-1 text-[11px] lg:text-sm font-medium text-gray-600 border-gray-400 focus:z-20 "
              v-html="link.label">

            </div>

            <Link :key="'link-' + key" v-else :href="link.url" v-html="link.label"
              class="relative inline-flex items-center border px-3 py-1 text-[11px] lg:text-sm rounded-sm"
              :class="link.active ? 'z-10 bg-secondarylight border-2 border-secondary text-secondary font-bold' : 'bg-gray-100 border-gray-400 text-black hover:bg-gray-200 hover:border-secondary'" />
          </template>
